...
bool IntelMausi::start(IOService *provider)
{
    ...
    IOLog("IntelMausiEthernet: Version %s starting on OS X Darwin %d.%d.\n", kmod_info.version, version_major, version_minor);
    ...
    if (!initPCIConfigSpace(pciDevice)) {
        IOLog("Ethernet [IntelMausi]: Failed initPCIConfigSpace(pciDevice).\n");
    ...
    if (!intelStart()) {
        IOLog("Ethernet [IntelMausi]: Failed intelStart().\n");
        goto error2;
    }    
    if (!setupMediumDict()) {
...
//    {
//
//        debugCursor = IOMbufNaturalMemoryCursor::withSpecification(0x800, 1);
//
//        if(debugCursor==NULL){
//            IOLog("Ethernet [IntelMausi]:  fKDPMbuf allocatePacket failed\n");
//        }
//        else{
//            unsigned int ret=  debugCursor->getPhysicalSegments(this-> fKDPMbuf , &debugvector, 0);
//             IOLog("Ethernet [IntelMausi]: debugCursor->getPhysicalSegments %d\n",ret);
//
//        }
//    }
    this->setProperty("location", "1");
    this->attachDebuggerClient(&debuger);
    IOLog("Ethernet [IntelMausi]: attachDebuggerClient( %p) \n",debuger);
...
{
    IOLog("IntelMausi  enable(IOKernelDebugger * debugger) <===\n");
    
     return IntelMausiEnable(1);

}

IOReturn IntelMausi::disable(IOKernelDebugger * debugger)
{
    IOLog("IntelMausi  disable(IOKernelDebugger * debugger) <===\n");
     return IntelMausiDisable(1);
...
void IntelMausi::sendPacket(void * pkt, UInt32 pktSize)
{
 ...
 //   IOReturn result = kIOReturnNoResources;
    ...
    if (!(iEnabled && linkUp) || forceReset) {
        DebugLog("Ethernet [IntelMausi]:sendPacket  Interface down. Dropping packets.\n");
        goto done;
    }
   ...
   // m =fKDPMbuf;
    m =allocatePacket(pktSize);
    memcpy(mbuf_data(m), pkt, pktSize);
    mbuf_setlen(m, pktSize);
    
    //&& (interface->dequeueOutputPackets(1, &m, NULL, NULL, NULL) == kIOReturnSuccess)
    ...
    {
          DebugLog("Ethernet [IntelMausi]: sendPacket have no txNumFreeDesc %u,  failed! \n", txNumFreeDesc);
       return;
    }
...
IOReturn  IntelMausi::IntelMausiEnable(int enbaled)
{
    IOReturn result = kIOReturnError;
    SInt32 score;
    IOLog("Ethernet [IntelMausi]  enable() ===>\n");
    
    if (iEnabled) {
        IOLog("Ethernet [IntelMausi]: Interface already enabled.\n");
...
IOReturn  IntelMausi::IntelMausiDisable(int enabled)
{
    IOReturn result = kIOReturnSuccess;
    
    IOLog("Ethernet [IntelMausi] disable() ===>\n");
